...
vol = 0.4 #Level of distractor attenuation relative to targets -- changed from 0.3 (ST3MA).
vol2 = 1 #Level of probe attenuation relative to targets -- changed from 0.45 (ST3MA).
...
item_max = np.amax(amax_array) #Max amplitude in sound_array.
max_peak_letter = sound_key[amax_array.index(item_max)]

#Adjust probe volume relative to distractors
sound_array[sound_key.index(u'4000')] = sound_array[sound_key.index(u'4000')] * (vol2/vol)

tone_stim = sound_key
print tone_stim
sounds_for_this_trial = list()
for i in range(np.size(tone_stim)): #this is giving me an error because 
    sounds_for_this_trial.append(sound_key.index(unicode(tone_stim[i])))
    if i == 0:
        stim_stream = sound_array[sounds_for_this_trial[i],:]
    else:
        stim_stream = np.concatenate((stim_stream, sound_array[sounds_for_this_trial[i],:]), axis=0)
RAPstream = sound.Sound(0.99*stim_stream)

RAPstream.play()

core.wait(RAPstream.getDuration() + 0.5)

#Finally, plot the played waveform
import pylab
pylab.plot(0.99*stim_stream)
pylab.show()
